const baseUrl =
  "https://us-central1-aizuhack-353413.cloudfunctions.net/Data?collection=teacher";
...
function add_star() {
  ...
  fetch(baseUrl + "&id=" + myId + "&key=rateAverage", {
    //評価平均データを格納
    method: "PATCH",
    body: JSON.stringify(starCountSum)
  });
  fetch(baseUrl + "&id=" + myId + "&key=ratedCount", {
    //評価回数データを格納
    method: "PATCH",
...
function add() {
  ...
  fetch(baseUrl + "&id=" + myId + "&key=comments", {
    //コメントデータを格納
    method: "PATCH",
